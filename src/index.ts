...
function renderDashboard(user: any) {
  ...
  <body>
    ...
    <script>
      ...
      function t(key) {
         const lang = (app && app.data && app.data.settings && app.data.settings.lang) ? app.data.settings.lang : 'en';
      ...
      const app = {
        ...
        async init() {
          try {
            ...
            const dateInput = document.getElementById('pay-analysis-date');
        ...
        nav(v) {
          ...
          let html = '';
          if(this.can('home')) html += \`<div class="nav-item \${v==='home'?'active':''}" onclick="app.nav('home')">\${t('over')}</div>\`;
          if(this.can('members')) html += \`<div class="nav-item \${v==='members'?'active':''}" onclick="app.nav('members')">\${t('mem')}</div>\`;
          if(this.can('attendance')) html += \`<div class="nav-item \${v==='attendance'?'active':''}" onclick="app.nav('attendance')">\${t('att')}</div>\`;
          if(this.can('history')) html += \`<div class="nav-item \${v==='history'?'active':''}" onclick="app.nav('history')">\${t('hist')}</div>\`;
          if(this.can('payments')) html += \`<div class="nav-item \${v==='payments'?'active':''}" onclick="app.nav('payments')">\${t('pay')}</div>\`;
          if(this.can('settings')) html += \`<div class="nav-item \${v==='settings'?'active':''}" onclick="app.nav('settings')">\${t('set')}</div>\`;
          if(currentUser.role==='admin') html += \`<div class="nav-item \${v==='users'?'active':''}" onclick="app.nav('users')">\${t('user')}</div>\`;
          nav.innerHTML = html;
        ...
        updateLabels() {
           setText('page-title', t('dash'));
           setText('btn-quick-checkin', t('quick_chk'));
           setText('lbl-active-mem', t('act_mem'));
           setText('lbl-today-visits', t('tod_vis'));
           setText('lbl-tot-rev', t('tot_rev'));
           setText('lbl-mem-due', t('mem_due'));
           setText('lbl-due-overview', t('due_ov'));
           
           const search = document.getElementById('search'); if(search) search.placeholder = t('search_ph');
           setText('btn-add-mem', t('add_mem'));
           setText('th-name', t('nm'));
           setText('th-phone', t('ph'));
           setText('th-plan', t('pl'));
           setText('th-exp', t('exp'));
           setText('th-due', t('due'));
           setText('th-act', t('act'));
           
           setText('lbl-today-att', t('tod_att'));
           setText('th-time', t('time'));
           
           setText('lbl-act-log', t('act_log'));
           
           setText('lbl-search-col', t('search_col'));
           setText('lbl-pay-stat', t('pay_stat'));
           
           setText('lbl-sys-set', t('sys_set'));
           setText('lbl-cur', t('cur'));
           setText('lbl-lang', t('lang'));
           setText('lbl-att-th', t('att_th'));
           setText('lbl-inact-th', t('inact_th'));
           setText('lbl-mem-plans', t('mem_plans'));
           setText('btn-add-plan', t('add_plan'));
           setText('btn-save-set', t('save_set'));
           
           setText('lbl-user-acc', t('user_acc'));
           setText('btn-add-user', t('add_user'));
           
           setText('lbl-chk-title', t('chk_title'));
           setText('btn-sub-chk', t('submit'));
           setText('lbl-new-mem', t('new_mem'));
           setText('lbl-rec-pay', t('rec_pay'));
           setText('txt-logout', t('Sign Out') + " →");
        },
