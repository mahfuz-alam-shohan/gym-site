    ...
    <script>
      ...
      const app = {
        ...
        renderMenuCheckboxes(containerId, selectedMenus = []) {
          ...
          box.innerHTML = this.menuOptions.map(opt => `<label style="display:inline-flex; align-items:center; gap:6px; background:#f9fafb; border:1px solid var(--border); padding:6px 10px; border-radius:8px; font-size:12px;"><input type="checkbox" value="${opt.key}" ${selected.includes(opt.key) ? 'checked' : ''}>${opt.label}</label>`).join('');
        },
